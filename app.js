...
function statusLabel(s){return{paid:"✓ Lunas",issued:"Jatuh tempo hari ini",arrears:"Menunggak",future:"Belum jatuh tempo",inactive:"Belum bisa dibayar"}[s]}
function billingCycleIsOpen(year,month,now=new Date()){
  const current=monthKey(now.getFullYear(),now.getMonth()),nextDate=new Date(now.getFullYear(),now.getMonth()+1,1),next=monthKey(nextDate.getFullYear(),nextDate.getMonth()),period=monthKey(year,month);
  return period===current||period===next;
}
...
function humanDateID(value){
  const m=String(value||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);if(!m)return value||'—';return `${Number(m[3])} ${MONTHS[Number(m[2])-1]} ${m[1]}`;
}
async function renderCustomerTable(){
  ...
  if(statusFilter!=='all'){
    customers=customers.filter(c=>{const s=mobileStateOf(c);return statusFilter==='paid'?s==='paid':statusFilter==='arrears'?s==='arrears':s==='issued'||s==='arrears'||(s==='future'&&billingCycleIsOpen(selectedYear,mobileSelectedMonth))});
  }
  ...
  for(let i=0;i<page.length;i++){
    ...
    for(let m=0;m<12;m++){
      ...
      const isPayablePeriod=billingCycleIsOpen(selectedYear,m);
      const b=document.createElement("button");b.className=`month-btn ${st}`;b.textContent=st==="future"&&!isPayablePeriod?"Belum bisa dibayar":statusLabel(st);b.dataset.cid=c.id;b.dataset.month=m;
    ...
    const mobileState=customerMonthState(c,selectedYear,mobileSelectedMonth,paidSet),pay=mobileState==='paid'?await paymentFor(c.id,monthKey(selectedYear,mobileSelectedMonth)):null,due=ymdLocal(dueDate(c,selectedYear,mobileSelectedMonth)),card=document.createElement('article');
    const payable=billingCycleIsOpen(selectedYear,mobileSelectedMonth),stateText=mobileState==='future'&&!payable?'Belum bisa dibayar':statusLabel(mobileState);
    card.className='mobile-customer-card';card.innerHTML=`<span class="mobile-row-number">${start+i+1}</span><button class="mobile-customer-name" type="button"><b>${c.name}</b><small>${c.customerCode}</small></button><div class="mobile-package"><b>${c.speed}</b><small>${money(c.monthlyPrice)}</small></div><button class="mobile-month-status ${mobileState}" type="button"><b>${stateText}</b><small>${humanDateID(pay?.date||due)}</small></button><button class="mobile-more" type="button">⋮</button>`;
    ...
    const sb=card.querySelector('.mobile-month-status');if(mobileState==='inactive'||(mobileState==='future'&&!payable))sb.disabled=true;else sb.addEventListener('click',()=>openPayment(c.id,mobileSelectedMonth));mobileList.appendChild(card);
  }
...
async function renderCustomerFocus(matches){
  ...
  const c=matches[0],ctx=await invoiceContextForCustomer(c),arrears=ctx.outstanding.filter(x=>x.status==='arrears'),unpaid=ctx.outstanding;
  const rows=unpaid.map(x=>`<li class="${x.status}"><span><b>${periodLabel(x.billingPeriod)}</b><small>Jatuh tempo ${humanDateID(x.billingDate)}</small></span><strong>${money(x.amount)}</strong></li>`).join('');
  box.innerHTML=`<div class="focus-card search-result-card"><div class="focus-profile"><div class="focus-avatar">${String(c.name||'?').trim().charAt(0).toUpperCase()}</div><div><span class="eyebrow">HASIL PENCARIAN</span><h3>${c.name}</h3><p>${c.customerCode} · WA ${c.whatsapp||'belum diisi'}</p></div><button class="focus-detail-btn" type="button">Detail ›</button></div><div class="focus-info-grid"><div><span>Paket</span><b>${c.packageName} · ${c.speed}</b></div><div><span>Tarif</span><b>${money(c.monthlyPrice)}</b></div><div class="warn"><span>Belum dibayar</span><b>${unpaid.length} bulan · ${money(ctx.outstandingTotal)}</b></div><div class="danger"><span>Menunggak</span><b>${arrears.length} bulan</b></div></div><div class="focus-unpaid"><div class="focus-unpaid-title"><b>Rincian tagihan belum dibayar</b><span>${unpaid.length} bulan</span></div>${rows?`<ul>${rows}</ul>`:'<div class="focus-clear">✓ Tidak ada tagihan yang belum dibayar</div>'}</div></div>`;
  box.querySelector('.focus-detail-btn').addEventListener('click',()=>openCustomerDetail(c.id));
}
...
async function deriveCurrentFilesForV8(st,events){
  ...
  const pkgBySyncKey=new Map(packages.map(p=>[String(p.syncKey||''),p]));
  for(const c of customers.filter(x=>x.active!==false)){
    const pkg=pkgBySyncKey.get(String(c.packageSyncKey||''))||null;
    const packageId=pkg?.packageCode||null,packageSyncKey=c.packageSyncKey||pkg?.syncKey||null,packageName=c.packageName||pkg?.name||'',speed=c.speed||pkg?.speed||'';
    const packageSnapshot={packageId,packageSyncKey,packageName,speed};
    const ctx=await invoiceContextForCustomer(c,new Date(),pref),r=curMap.get(c.customerCode)||{},out=ctx.outstanding.map(x=>({...x,...packageSnapshot,billingState:x.status,status:'unpaid'})),payable=ctx.payable.map(x=>({...x,...packageSnapshot,billingState:x.status,status:'unpaid'})),latest=ctx.latestIssued?{...ctx.latestIssued,...packageSnapshot}:null;
    botCustomers.push({invoiceId:r.invoiceId||ctx.current?.invoiceId||null,customerId:c.customerCode,name:c.name,whatsapp:c.whatsapp||'',packageId,packageSyncKey,packageName,speed,billingPeriod:r.billingPeriod||ctx.current?.billingPeriod||null,usagePeriod:r.usagePeriod||ctx.current?.usagePeriod||null,billingDate:r.billingDate||ctx.current?.billingDate||null,amount:Number(r.amount||ctx.current?.amount||c.monthlyPrice||0),status:r.status==='paid'?'paid':'unpaid',paymentDate:r.paymentDate||null,paymentStateEventId:r.paymentStateEventId||null,paymentStatusAt:r.paymentStatusAt||null,paymentSource:r.paymentSource||null,paymentCycle:Number(r.paymentCycle||0),registrationDate:c.registrationDate||null,startDate:c.startDate||null,firstBillDate:c.firstBillDate||null,billingDay:new Date(c.firstBillDate+'T00:00:00').getDate(),outstandingInvoices:out,outstandingCount:ctx.outstandingCount,outstandingTotal:ctx.outstandingTotal,outstandingInvoiceIds:out.map(x=>x.invoiceId),latestOutstandingInvoice:out.at(-1)||null,earlyPaymentInvoices:payable,earlyPaymentCount:ctx.payableCount,earlyPaymentTotal:ctx.payableTotal,latestIssuedInvoice:latest,pascaBayarExample:ctx.example,dynamicMessage:{mode:'adaptive-single-multiple',unpaid:{invoiceCount:ctx.outstandingCount,total:ctx.outstandingTotal,invoices:out,example:ctx.example}}});
  }
